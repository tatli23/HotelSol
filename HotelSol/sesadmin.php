...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $input_password = trim($_POST['contrasena'] ?? '');
    ...
    if (empty($input_usuario) || empty($input_password)) {
        ...
        try {
            $stmt = $pdo->prepare("SELECT id, usuario, contrasena FROM administrador WHERE usuario = ? LIMIT 1");
            $stmt->execute([$input_usuario]);
            $result = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($result && $input_password === $result['contrasena']) {
...
// Si hay error o no es POST, mostrar el formulario
?>
    ...
    <div class="container">
        ...
        <div class="right-panel">
            ...
            <form class="login-form" id="admin-form" method="post" action="sesadmin.php">
                ...
                <div class="form-group">
                    <label class="form-label">Contraseña</label>
                    <input type="password" class="form-input" name="contrasena" required>
                </div>
